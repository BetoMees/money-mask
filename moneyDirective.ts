...
export class MoneyDirective {
  ...
  private set(inputValue) {
    return inputValue === null && this._inputDefault != null
      ? this._inputDefault
      : inputValue
  }
  ...
  private init(returnValue = true) {
    if (
      ...
      (this._type === undefined || this._type === null)
    ) {
  ...
  private makeNegative(inputValue: string): string {
    return this._isNegative &&
      this._canNegative &&
      parseFloat(this.removeChars(inputValue)) != 0
      ? '-'.concat(inputValue)
      : inputValue
  }
  ...
  private toFixed(inputValue: string): string {
    ...
    return isNaN(value)
      ? null
      : value.toFixed(this._decimal).replace('.', this._decimalMarker)
  }
  ...
  private applyDecimal(inputValue: string): string {
    if (this._decimal > 0) {
      ...
      inputValue = inputValue
        .substring(0, pos)
        .concat('.', inputValue.substr(pos, this._decimal))
    }
    return inputValue
  }
